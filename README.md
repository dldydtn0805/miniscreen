...
- 헤더 또는 주소 입력창 드래그 이동, 우하단 핸들 리사이즈 지원
...
- 헤더 또는 주소 입력창을 드래그해 위치 이동
- 우하단 리사이즈 핸들로 크기 조절
- 주소 입력창 더블클릭으로 전체화면 토글
...
- 새 탭/팝업을 유도하는 링크 target, `window.open()`, `form target` 제출을 가능한 한 iframe 내부 탐색으로 전환
...
- 알려진 외부 광고 스크립트 도메인은 동적 규칙으로 차단할 수 있습니다.
