...
├── background.js      # 아이콘 클릭 처리, 동적 네트워크 규칙, 뷰 모드 저장
├── content.js         # 미니 화면 UI 생성 및 사용자 인터랙션 처리
├── frame-tracker.js   # iframe 내부 주소 변경 추적 및 링크 이동 보정
...
- 확장 아이콘 클릭 시 `content.js`를 현재 탭에 주입
- `declarativeNetRequest` 동적 규칙으로 응답 헤더 수정
- 모바일 뷰일 때 서브프레임 요청에 모바일 User-Agent 적용
- 뷰 모드 변경 메시지를 받아 저장하고 규칙 갱신

### content.js

- `#miniscreen` 오버레이 DOM 생성
- 헤더, 검색창, 북마크 패널, iframe, 리사이즈 핸들 구성
- 북마크 / 홈 URL / 뷰 모드 로드 및 저장
- 드래그 이동, 리사이즈, 탐색 버튼, 뷰 모드 버튼 처리
...
- `scripting`: 현재 탭에 `content.js`를 주입하기 위해 사용
